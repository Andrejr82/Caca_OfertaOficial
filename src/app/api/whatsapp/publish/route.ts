...
export async function POST(request: Request) {
  try {
    ...
    const idempotencyKey = body.idempotencyKey ?? publicationIdempotencyKey(body.postId, "whatsapp", commandId);
    ...
    const command: OfficialPublicationCommand = {
      ...
      offerId: body.offerId, postId: body.postId, tenantId: user.id, channel: "whatsapp",
      ...
      origin: "publication.whatsapp.route", reason: { code: "USER_REQUESTED_PUBLICATION" },
      metadata: { requestSource: body.requestSource ?? "whatsapp-dashboard" }
    };
