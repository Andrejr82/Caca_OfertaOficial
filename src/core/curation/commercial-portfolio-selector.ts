...
const TYPE_PATTERNS: readonly [string, RegExp][] = [
  ...
  ["racao", /\bracao\b/], ["cama-pet", /\b(cama|caminha)\b.*\b(pet|cachorro|gato)/], ["arranhador", /\barranhador\b/],
  ["bebedouro-pet", /\bbebedouro\b.*\b(pet|cao|caes|gato)/], ["mordedor-pet", /\bmordedor\b.*\b(cachorro|pet)/],
  ["tenis", /\btenis\b/], ["notebook", /\b(notebook|laptop)\b/], ["smartphone", /\b(celular|smartphone|iphone|galaxy)\b/],
  ["fone", /\b(fone|headphone|headset|earbuds?)\b/], ["smartwatch", /\b(smartwatch|relogio inteligente)\b/],
...
function parseComparableAmount(title: string, type: string): { kind: CommercialPortfolioRankedOffer["measureKind"]; amount: number } {
  ...
  if (multipack) {
    const count = number(multipack[1]), value = number(multipack[2]), unit = multipack[3];
...
function normalizedTokens(title: string): Set<string> {
  return new Set(normalize(title).split(" ").filter((token) => token.length > 2 && !STOP_WORDS.has(token) && !/^\d+(?:\.\d+)?$/.test(token)).filter((token) => !/^(kg|ml|cm|mm|un|unid|unidade|unidades)$/.test(token)));
}
...
function scoreRows(offers: readonly CommercialPortfolioOffer[]): CommercialPortfolioRankedOffer[] {
  const prepared = offers.filter((offer) => offer.id && number(offer.current_price) > 0 && offer.status !== "rejected").map((offer) => {
    const type = commercialType(offer), amount = parseComparableAmount(offer.product_name ?? "", type), current = number(offer.current_price);
    return { offer, commercialType: type, measureKind: amount.kind, comparablePrice: current / Math.max(amount.amount, 1) };
  });
  const minByGroup = new Map<string, number>(), countByGroup = new Map<string, number>();
  ...
  return prepared.map((row) => {
    const m = metrics(row.offer), group = `${row.commercialType}:${row.measureKind}`;
    const groupCount = countByGroup.get(group) ?? 1, minPrice = minByGroup.get(group) ?? row.comparablePrice;
    const pricePoints = groupCount > 1 ? 30 * Math.min(1, minPrice / row.comparablePrice) : 12;
    const discountPct = explicitDiscountPercent(row.offer), discountPoints = Math.min(20, (discountPct / 50) * 20);
    const sales = number(m.sales), reviews = number(m.reviewCount), proofBase = Math.max(sales, reviews);
    const socialPoints = proofBase > 0 ? Math.min(20, (Math.log10(proofBase + 1) / 4.2) * 20) : 0;
    const rating = number(m.rating) || number(m.sellerRating), ratingPoints = rating > 0 ? Math.min(10, (rating / 5) * 10) : 0;
    const commission = number(m.commissionRate), commissionPoints = commission > 0 ? Math.min(10, (commission / 20) * 10) : 0;
    const position = number(m.sourcePosition) || number(m.position), positionPoints = position > 0 ? (position <= 3 ? 5 : position <= 10 ? 3.5 : 1.5) : 0;
    const trustPoints = m.officialStoreId || m.prime || m.coupon ? 5 : 0;
    ...
    return { ...row, score: total, reasons: Object.freeze([`price=${pricePoints.toFixed(1)}`, `discount=${discountPoints.toFixed(1)}`, `social=${socialPoints.toFixed(1)}`, `rating=${ratingPoints.toFixed(1)}`, `commission=${commissionPoints.toFixed(1)}`]) };
  }).sort((a, b) => b.score - a.score || a.offer.id.localeCompare(b.offer.id));
}

export function selectCommercialPortfolio(offers: readonly CommercialPortfolioOffer[], options: CommercialPortfolioOptions = {}): CommercialPortfolioSelection {
