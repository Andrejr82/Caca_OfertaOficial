...
function rejected(
  ...
  return { status: "rejected", ...command, code, message, failureStage };
}
...
export async function approveOfficialOfferForPublication(
  ...
  if (offerState === "pending_manual_review") {
    ...
    if (selection.status === "rejected") return rejected(command, selection.code, selection.message, "selection");
