...
describe("Mercado Livre Radar Discovery V1", () => {
  it("uses a compact official intent set covering at least five macro groups", () => {
    ...
    const macroGroups = new Set(
      ML_RADAR_DISCOVERY_INTENTS.map((intent: string) => ML_RADAR_INTENT_MACRO_GROUPS[intent]),
    );
      ...
      products: [
        {
          ...
          domain_id: "MLB-COMPUTER_MICE",
          category_id: "MLB1714",
          image_url: "https://http2.mlstatic.com/mouse.jpg",
          product_url: "https://www.mercadolivre.com.br/p/MLB-P1",
          source_position: 1,
        },
      ],
    }));
  ...
  it("deduplicates native items and excludes invalid prices without fabricating metrics", async () => {
    ...
    expect(result[0].commissionPercent).toBe(0);
  });
  ...
  it("preserves official discovery metadata during normalization", () => {
    const result = normalizeMercadoLivreDiscoveryProduct({
      ...
      current_price: 199,
      old_price: 249,
      intent: "mala de bordo 10kg",
      domain_id: "MLB-SUITCASES",
      category_id: "MLB417705",
      source_position: 2,
      image_url: "https://http2.mlstatic.com/mala.jpg",
      product_url: "https://www.mercadolivre.com.br/p/MLB-P9",
    });

    expect(result).toMatchObject({
      sourceIntent: "mala de bordo 10kg",
      macroGroup: "viagem",
      domainId: "MLB-SUITCASES",
      categoryId: "MLB417705",
      sourcePosition: 2,
      currentPrice: 199,
      oldPrice: 249,
    });
