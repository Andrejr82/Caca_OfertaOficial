...
async function run() {
  ...
  const commandId = process.env.COMMAND_ID || `admin-rest:${postId}:${channel}`;
  ...
  const command: OfficialPublicationCommand = {
    ...
    requestedAt: new Date().toISOString(), actor: { type: "service", id: "admin-script", service: "publish-rest" },
    origin: "scripts.publish-rest", reason: { code: "PUBLISH_OFFICIAL_POST" },
