/**
 * MOTOR DE LOCUÇÃO VIRAL (VOICEOVER ENGINE) - Versão CommonJS
 *
...
function cleanProductNameForVoiceover(name) {
  return String(name || "")
...
function shortenProductTitleForSpeech(name) {
...
function estimateVoiceoverDurationSeconds(script) {
...
function buildViralVoiceoverScript(offer, format, customCta) {
  const cta = (customCta && customCta.trim()) || DEFAULT_CTA;
  const rawTitle = (offer.short_name && offer.short_name.trim()) || (offer.product_name && offer.product_name.trim()) || "";
