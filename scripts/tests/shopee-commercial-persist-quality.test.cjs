...
test('preserva produto com rating abaixo de 4.7 tecnicamente elegível', () => assert.equal(controlledCandidateQuality(p({ ratingStar: 4.6 })).eligible, true));
test('preserva produto com vendas abaixo de 100 tecnicamente elegível', () => assert.equal(controlledCandidateQuality(p({ sales: 44 })).eligible, true));
...
test('preenche o limite com candidatos qualificados combinados', () => {
  const rows = [p({ itemId:'1', ratingStar:4.6 }), p({ itemId:'2', sales:50 }), p({ itemId:'3' }), p({ itemId:'4' }), p({ itemId:'5' })];
  const selected = selectControlledPersistCandidates(rows, { maxCandidates: 2 });
  assert.deepEqual(selected.map((x)=>x.itemId), ['1','2']);
});
