...
test('5. Família não mapeada usa descoberta dinâmica API-first sem rota legada', async () => {
  ...
  const result = await runMercadoLivreOfficialIntentCoverage({
    ...
    fetchImpl: async (url) => {
      const value = String(url); calls.push(value);
      if (value.includes('/domain_discovery/search')) return json([
        { domain_id: 'MLB-BRINQUEDOS', category_id: 'MLB1132', category_name: 'Brinquedos' },
        ...Array.from({ length: 9 }, (_, index) => ({ domain_id: `MLB-COFFEE_MAKERS_${index}`, category_id: `MLB1576${index}`, category_name: 'Cafeteiras' })),
      ]);
