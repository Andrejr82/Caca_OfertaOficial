...
async function crawleeExtract(url, limit, storeName) {
  ...
  if (storeName === 'Mercado Livre' && SCRAPFLY_KEYS.length > 0) {
    ...
    // Utilizando API Direta do Scrapfly retornando o HTML cru (format=raw) com renderização de JS e proxy residencial
    targetUrl = `https://api.scrapfly.io/scrape?key=${key}&url=${encodeURIComponent(url)}&asp=true&country=br&render_js=true&proxy_pool=public_residential_pool&format=raw`;
