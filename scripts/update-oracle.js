...
const sshCommandGit = `ssh -i ${SSH_KEY_PATH} ${SERVER_USER}@${SERVER_IP} "cd ${PROJECT_DIR} && git pull origin main && npm install && pm2 restart ${PM2_APP_NAME}"`;
...
// const sshCommandScp = `scp -i ${SSH_KEY_PATH} scripts/oracle-api.cjs ${SERVER_USER}@${SERVER_IP}:${PROJECT_DIR}/scripts/oracle-api.cjs && ssh -i ${SSH_KEY_PATH} ${SERVER_USER}@${SERVER_IP} "pm2 restart ${PM2_APP_NAME}"`;
