...
function getShopeeFirstDiscoveryDeepeningPlan(scenarioId, env = process.env) {
  if (getFirstDiscoveryQualityMode(env) !== 'active') {
    return Object.freeze({ enabled: false, reason: 'first_discovery_not_active' });
  }
  ...
  if (!firstDiscovery?.intents?.length) {
    return Object.freeze({ enabled: false, reason: 'first_discovery_plan_missing' });
  }
    ...
    for (let page = 1; page <= plan.limits.maxPagesPerQuery; page += 1) {
      ...
      try {
        response = await request(
          ...
          { keyword, page, limit: plan.limits.pageSize, sortType: 2, isAMSOffer: true },
          { signal },
        );
      } catch (error) {
      ...
      if (nodes.length === 0) {
        calls.push({ ...evidence, stopReason: 'empty_page' });
        break;
      }
      if (pageInfo?.hasNextPage !== true) {
        calls.push({ ...evidence, stopReason: 'has_next_page_false' });
        break;
      }
...
function mergeMetrics(primary = {}, supplemental = {}, finalCount = 0) {
  ...
  for (const key of ['raw', 'parsed', 'normalized', 'technicalRejected', 'intentRejected', 'approvedContract', 'duplicates', 'scoreable']) {
    merged[key] = Number(primary?.[key] || 0) + Number(supplemental?.[key] || 0);
  }
  ...
  for (const [reason, count] of Object.entries(supplemental?.rejections || {})) {
    merged.rejections[reason] = Number(merged.rejections[reason] || 0) + Number(count || 0);
  }
  return merged;
}
...
async function runShopeeFirstDiscoveryDeepening(scenarioId, options = {}) {
  const primary = await runScenarioPlan(scenarioId, {
  ...
  if (mergedTop.length < plan.minCandidates && !options.signal?.aborted) {
    [delta, auxiliary] = await Promise.all([
      collectDelta({ request: options.request, plan, signal: options.signal }),
      collectAuxiliary({ request: options.request, plan, signal: options.signal }),
    ]);
    supplemental = runSupplemental(scenarioId, {
      productOffers: intent.products,
      deltaRows: delta.rows,
      datafeedId: delta.datafeedId,
      shopOffers: auxiliary.shopOffers,
      shopeeOffers: auxiliary.shopeeOffers,
      maxFeedRows: plan.limits.maxFeedRows,
    });
  ...
  const scenarios = {
    ...
    [scenarioId]: {
      ...primaryScenario,
      top: mergedTop,
      rejected: [...(primaryScenario.rejected || []), ...(supplementalScenario.rejected || [])],
      metrics: mergeMetrics(primaryScenario.metrics || {}, supplementalScenario.metrics || {}, mergedTop.length),
    },
  };
  const calls = [...(primary.queryEvidence?.calls || []), ...intent.calls, ...delta.calls, ...auxiliary.calls];
  return {
    ...primary,
    scenarios,
    queryEvidence: {
      ...
      calls,
      productOffers: Number(primary.queryEvidence?.productOffers || 0) + intent.products.length,
      deltaRows: delta.rows.length,
      shopOffers: auxiliary.shopOffers.length,
      shopeeOffers: auxiliary.shopeeOffers.length,
    },
    deepening: {
      applied: true,
      reason: 'primary_pool_below_minimum',
      nicheId: plan.nicheId,
      primaryTop: Number(primaryScenario.top?.length || 0),
      finalTop: mergedTop.length,
      targetCandidates: plan.targetCandidates,
      minCandidates: plan.minCandidates,
      queriesAttempted: plan.queries.length,
      intentProducts: intent.products.length,
      deltaRows: delta.rows.length,
      auxiliaryResolved: auxiliary.shopOffers.filter((item) => item?.resolved).length + auxiliary.shopeeOffers.filter((item) => item?.resolved).length,
    },
  };
}
...
module.exports = {
  DEFAULTS,
  getShopeeFirstDiscoveryDeepeningPlan,
  runShopeeFirstDiscoveryDeepening,
};
