...
function resolveIdentity(candidate={}){const marketplace=String(candidate.marketplace||'').trim(),raw=candidate.identityKey||candidate.itemId||candidate.productId||candidate.asin||candidate.productName||candidate.title||'';return `${marketplace}:${String(raw).trim()}`;}
