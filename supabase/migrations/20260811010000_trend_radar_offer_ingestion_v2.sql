-- Compatibilidade da ingestão Radar: novos produtos entram na fila manual;
-- registros existentes preservam a decisão editorial já tomada.
create or replace function public.upsert_trend_radar_offers_v2(
  p_marketplace text,
  p_rows jsonb
)
returns jsonb
...
begin
  ...
  loop
    ...
    if p_marketplace = 'Shopee' and v_shopee_item_id is not null then
      ...
      select id into v_existing_id from public.offers
       where user_id = v_user_id and platform = 'Mercado Livre'
         and ((v_item_id is not null and item_id = v_item_id)
           or (v_item_id is null and v_product_id is not null and product_id = v_product_id)) limit 1;
    end if;
  ...
  v_result := public.upsert_trend_radar_offers_v1(p_marketplace, p_rows);
...
revoke all on function public.upsert_trend_radar_offers_v2(text, jsonb) from public;
revoke all on function public.upsert_trend_radar_offers_v2(text, jsonb) from anon;
revoke all on function public.upsert_trend_radar_offers_v2(text, jsonb) from authenticated;
grant execute on function public.upsert_trend_radar_offers_v2(text, jsonb) to service_role;
