drop function if exists public.create_or_reuse_auto_reel_job(uuid, uuid, text, jsonb);

create function public.create_or_reuse_auto_reel_job(
  _user_id uuid,
  _offer_id uuid,
  _script text,
  _metadata jsonb
...
revoke all on function public.create_or_reuse_auto_reel_job(uuid, uuid, text, jsonb) from anon;
revoke all on function public.create_or_reuse_auto_reel_job(uuid, uuid, text, jsonb) from authenticated;
