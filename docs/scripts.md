...
|---|---|---|---|---|
| Oracle Scraper | `scripts/oracle-scraper.cjs` | Orquestrador de scraping. Shopee V5, com `SHOPEE_DISCOVERY_V5=true`, usa categorias nativas, `productOfferV2(productCatId)`, Top 20 por categoria e curadoria manual; IA e publicação somente após `selected`. Demais marketplaces preservam seus fluxos. | Execução contínua (processo longo) | Node, `crawlee`, `playwright`, `@supabase/supabase-js`, `axios`, `.env.local` |
