...
- Telegram, Instagram e WhatsApp são canais de posts no schema e em `Official Publication`. Facebook tem rota/transport, mas não aparece como canal permitido no schema-base de `posts`; não deve ser descrito como canal canônico persistido sem verificar a migration/ambiente correspondente.
...
O Oracle→Vercel é um POST para a URL configurada em `OFFICIAL_AI_TRIGGER_URL`, ou para a base pública resolvida por `APP_URL`, `NEXT_PUBLIC_APP_URL`, `PUBLIC_APP_URL`, `NEXTAUTH_URL`, `AUTH_URL` ou `VERCEL_PROJECT_PRODUCTION_URL`, sempre terminando em `/api/ai/generate`. O request usa bearer da service role quando disponível e timeout de 120 s. O Vercel→Oracle usa `POST /api/scrape` na porta 3002, autenticado por `ORACLE_API_KEY`; a API seleciona Scrapfly para não-Amazon e Scrape.do para Amazon. O endereço público/privado real não é certificado pelo código.
